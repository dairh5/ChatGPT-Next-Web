...
const Chat = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/openxlab', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          model: 'gpt-3.5-turbo', // 替换为 OpenXLab 支持的模型名称
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch response from OpenXLab');
      }
    } catch (error) {
      console.error('Error communicating with OpenXLab API:', error);
    }
  };
